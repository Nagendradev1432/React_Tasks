...
class Updating extends Component {
    ...
    componentDidUpdate = (prevProps, prevState, snapshot) => {
        ...
        // Safely store the snapshot into the `preve` state
        if (snapshot && snapshot.preve && snapshot.preve !== this.state.preve) {
            this.setState({ preve: snapshot.preve });
        }
    };

    getSnapshotBeforeUpdate(prevProps, prevState) {
        // Capture the previous product state for snapshot
